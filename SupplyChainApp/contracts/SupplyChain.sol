...
contract SupplyChain {
    ...
    function addParticipant(string memory _name, string memory _pass, address _pAdd, 
    ...
    function getParticipant(uint32 _product_id) public view returns (
    ...
    function addProduct(uint32 _ownerId, string memory _modelNumber, 
        string memory _partNumber, string memory _serialNumber, uint32 _productCost) 
        public returns (uint32) {
        // Have to compare the hash of the string in solidity to check if manufacturer, cannot compare directly.
    ...
    function getProduct(uint32 _productId) public view returns (
        string memory, string memory, string memory, uint32, address, uint32) {
    ...
    function newOwner(uint32 _user1Id, uint32 _user2Id, uint32 _prodId) 
        ...
        // Transfer from supplier to supplier.
        ...
        // Transfer from supplier to consumer.
        else if (keccak256(abi.encodePacked(p1.participantType)) == keccak256("Supplier")
    ...
    function authenticateParticipant(uint32 _uid, string memory _uname, 
        string memory _pass, string memory _utype) public view returns (bool) {
